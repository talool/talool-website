...
public class PublishersPanel extends BaseTabPanel {

	private static final long serialVersionUID = 2170124491668826388L;
	private static final Logger LOG = LoggerFactory.getLogger(PublishersPanel.class);
	private static final String CONTAINER_ID = "list";
	private static final String REPEATER_ID = "rptr";
	...
	public PublishersPanel(String id) {
		super(id);
	...
	protected void onInitialize() {
		...
		dataProvider.setFilterPublisher(true);
		...
		// Wizard
		wizard = new MerchantWizard("wiz", "Publisher Wizard", MerchantWizardMode.MERCHANT)
		{
	...
	private DataView<MerchantSummary> getDataView(IDataProvider<MerchantSummary> dataProvider)
	{
		final DataView<MerchantSummary> schools = new DataView<MerchantSummary>(REPEATER_ID,dataProvider)
		{
			...
			{
				final MerchantSummary publisher = item.getModelObject();
				final UUID publisherId = publisher.getMerchantId();

				item.setModel(new CompoundPropertyModel<MerchantSummary>(publisher));
				...
				booksParams.set("id", publisherId);
				booksParams.set("name", publisher.getName());
				...
				ExternalLink nameLink = new ExternalLink("name", Model.of(url),
						new PropertyModel<String>(publisher, "name"));
				item.add(nameLink);
				...
				item.add(new AjaxLink<Void>("editLink")
				{
					...
					{
						getSession().getFeedbackMessages().clear();
						wizard.setModelObject(new MerchantModel(publisherId, true).getObject());
						wizard.open(target);
					}
				});
				
				StringBuilder confirm = new StringBuilder();
				confirm.append("Are you sure you want to remove \"").append(publisher.getName()).append("\"?");
