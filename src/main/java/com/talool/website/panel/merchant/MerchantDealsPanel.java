...
public class MerchantDealsPanel extends BaseTabPanel
{
	...
	private static final long serialVersionUID = 9061565518088516731L;
		...
		final ListView<Deal> deals = new ListView<Deal>("dealRptr", model)
		{
			...
			protected void populateItem(ListItem<Deal> item)
			{
				...
				item.add(new AjaxLink<Void>("editLink")
				{
					...
					private static final long serialVersionUID = 5853739699900430448L;

					@Override
					public void onClick(AjaxRequestTarget target)
					{
						getSession().getFeedbackMessages().clear();
						MerchantDealPanel panel = new MerchantDealPanel(modal.getContentId(), callback, dealId);
	...
	public Panel getNewDefinitionPanel(String contentId, SubmitCallBack callback) {
		return new MerchantDealPanel(contentId, _merchantId, callback);
	}
