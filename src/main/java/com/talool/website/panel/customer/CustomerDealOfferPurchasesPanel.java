			...
			protected void populateItem(ListItem<DealOfferPurchase> item)
			{
				...
				if (props != null)
				{
					try
					{
						...
						{
							// TODO without a receipt, we don't know if the price has changed since the purchase date
							if (dop.getDealOffer().getType().equals(DealType.FREE_BOOK) || 
									dop.getDealOffer().getType().equals(DealType.FREE_DEAL) ||
									dop.getDealOffer().getPrice() == 0f)
							{
								price = "Free";
							}
						}
