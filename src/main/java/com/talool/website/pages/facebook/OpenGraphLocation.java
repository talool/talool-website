...
public class OpenGraphLocation extends OpenGraphRepeator {
	...
	protected void onInitialize() {
		...
		street.add(new AttributeModifier("content",location.getAddress().getAddress1()));
		add(street);
		
		WebMarkupContainer city = new WebMarkupContainer("city");
		city.add(new AttributeModifier("content",location.getAddress().getCity()));
		add(city);
		
		WebMarkupContainer state = new WebMarkupContainer("state");
		state.add(new AttributeModifier("content",location.getAddress().getStateProvinceCounty()));
		add(state);
		
		WebMarkupContainer zip = new WebMarkupContainer("zip");
		zip.add(new AttributeModifier("content",location.getAddress().getZip()));
		add(zip);
		
		WebMarkupContainer country = new WebMarkupContainer("country");
		country.add(new AttributeModifier("content",location.getAddress().getCountry()));
		add(country);
